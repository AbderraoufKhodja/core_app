...
class SwapItScreen extends StatefulWidget {
  const SwapItScreen({super.key});

  @override
  SwapItScreenState createState() => SwapItScreenState();
}

class SwapItScreenState extends State<SwapItScreen>
    with AutomaticKeepAliveClientMixin<SwapItScreen>, TickerProviderStateMixin {
  @override
  ...
  void initState() {
    ...
    if (!_settingsCubit.state.isSwapScreenShowCaseDone) {
      Future.delayed(const Duration(seconds: 15), () {
        if (ModalRoute.of(context)?.isCurrent == true && _settingsCubit.activeIdx == 0) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Row(
        ...
        children: [
          const Expanded(child: SwapCardsDecksWidget()),
          AnimatedContainer(
