...
class VideoPostInfoPage extends StatefulWidget {
  const VideoPostInfoPage({super.key});

  @override
  VideoPostInfoPageState createState() => VideoPostInfoPageState();
}

class VideoPostInfoPageState extends State<VideoPostInfoPage>
    with AutomaticKeepAliveClientMixin<VideoPostInfoPage> {
  ...
  void initState() {
    ...
    _keywords = _videoPostFactoryBloc.post.tags?.cast<String>() ?? [];
  ...
  List<Widget> get widgets => <Widget>[
        _videoPostFactoryBloc.post.isActive == false
            ? ListTile(
                title: Text(RCCubit.instance.getText(R.restoreItem)),
                trailing: const FaIcon(FontAwesomeIcons.circleInfo),
                onTap: () {
                  _videoPostFactoryBloc.handleRestoreStorePost(
                    context,
                    itemID: _videoPostFactoryBloc.post.postID!,
                  );
                },
              ...
              onPressed: () {
                if (_tagsController.text.isNotEmpty) {
                  setState(() {
                    ...
                    _videoPostFactoryBloc.post.tags = _keywords;
  ...
  Widget _buildAddImagesCard() {
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        suffixIcon: (_videoPostFactoryBloc.post.photoUrls is List<dynamic> &&
            ...
            ? GridView.builder(
                ...
                itemBuilder: (context, index) {
                  final dynamicPhoto = _videoPostFactoryBloc.post.photoUrls![index];
                  return ClipRRect(
                ...
                itemCount: _videoPostFactoryBloc.post.photoUrls!.length,
              )
            : Padding(
