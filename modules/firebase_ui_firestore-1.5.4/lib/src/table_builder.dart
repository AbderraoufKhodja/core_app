...
class _FirestoreTableState extends State<FirestoreDataTable> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: _query.get(const GetOptions(source: Source.server)),
                ...
                return AnimatedBuilder(
                  ...
                  builder: (context, child) {
                    ...
                    return PaginatedDataTable(
                      ...
                      columns: [
                        for (final head in widget.columnLabels.values) DataColumn(label: head)
                      ],
                    );
                  },
                );
...
class _PropertyTypeForm extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (formState is _NumberFormState) {
      ...
      return Checkbox(
        onChanged: (_) => onFormStateChange(_BooleanFormState(!formState.value)),
...
class _NumberFormState extends _FormState {
  _NumberFormState(String text) : controller = TextEditingController(text: text);
...
class _StringFormState extends _FormState {
  _StringFormState(String text) : controller = TextEditingController(text: text);
...
class _ReferenceFormState extends _FormState {
  _ReferenceFormState(String text) : controller = TextEditingController(text: text);
...
class _Source extends DataTableSource {
      ...
      onSelectChanged: selectionEnabled
          ? (selected) {
              ...
              if ((selected && _selectedRowIds.add(doc.id)) ||
                ...
                onSelectedRows?.call(
                  _previousSnapshot!.docs.where((e) => _selectedRowIds.contains(e.id)).toList(),
                );
  ...
  void setFromSnapshot(
    ...
    final wereAllItemsSelected = _previousSnapshot?.docs.length == _selectedRowIds.length &&
...
class _ValueView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (value == null) {
      ...
      final localizations = FirebaseUILocalizations.labelsOf(context);
      final latitudeLabel =
          value.latitude < 0 ? localizations.southInitialLabel : localizations.northInitialLabel;
      final longitudeLabel =
          value.longitude < 0 ? localizations.westInitialLabel : localizations.eastInitialLabel;
