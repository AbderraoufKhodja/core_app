...
class _FirestoreQueryBuilderState<Document> extends State<FirestoreQueryBuilder<Document>> {
  Future<QuerySnapshot<Document>>? _querySubscription;
  ...
  _fetchQuery({bool nextPage = false}) async {
    ...
    try {
      // TODO initial cache data need to be tested
      ...
      setState(() {
        ...
        _snapshot = _snapshot.copyWith(
          hasData: true,
          docs: querySnapshot.size < expectedDocsCount
              ? querySnapshot.docs
              : querySnapshot.docs.take(expectedDocsCount - 1).toList(),
          error: null,
          hasMore: querySnapshot.size == expectedDocsCount,
          stackTrace: null,
          hasError: false,
        );
      });
    ...
    query.get(const GetOptions(source: Source.server)).then(
      (event) {
        setState(() {
          if (nextPage) {
            _snapshot = _snapshot.copyWith(isFetchingMore: false);
...
abstract class FirestoreQueryBuilderSnapshot<Document> {
  ...
  /// Try to the query.
  ///
  /// this will not take into account fetching state
  void reloadPage();
}
