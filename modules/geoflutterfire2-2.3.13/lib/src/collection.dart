...
class GeoFireCollectionRef {
  ...
  //       Map<dynamic, dynamic> snapData = distanceDocSnapshot.documentSnapshot.exists
  ...
  Query closest({
    ...
    // final precision = Util.setPrecision(radius);
    // precision: 5, blockNumber: 20 => 100 km

    final precision = 5;
    final blockNumber = 18;
    final centerHash = center.hash.substring(0, precision);
    final areas = GeoFirePoint.neighborsOf(hash: centerHash)
  ...
  /// query firestore documents based on geographic [radius] from geoFirePoint [center]
  /// [field] specifies the name of the key in the document
  Query precisionFiveBlockSixtyThree({
    required GeoFirePoint center,
    required double radius,
    required String field,
    bool strictMode = false,
    ...
    // final precision = Util.setPrecision(radius);
    // precision: 5, blockNumber: 63 => 302 km

    final precision = 5;
    final blockNumber = 63;
    final centerHash = center.hash.substring(0, precision);
    final areas = GeoFirePoint.threeHundredKMOf(hash: centerHash)
      ..addEntries([MapEntry('block0', centerHash)]);
  ...
  Query _unboundedGeoQuery({
    ...
    final temp = _collectionReference;
    // compensate for idx start at 0
    final idx = precision - 1;
    return temp.where(
      '$field.data.precision$idx.block$blockNumber',
      arrayContainsAny: areas['block1'],
    );
  }
