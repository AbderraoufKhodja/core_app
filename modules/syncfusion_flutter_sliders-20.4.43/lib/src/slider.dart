...
class SfSlider extends StatefulWidget {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties
        .add(ObjectFlagProperty<ValueChanged<double>>('onChanged', onChanged, ifNull: 'disabled'));
    ...
    properties.add(FlagProperty('showTicks',
        value: showTicks, ifTrue: 'Ticks are showing', ifFalse: 'Ticks are not showing'));
    properties.add(FlagProperty('showLabels',
        value: showLabels, ifTrue: 'Labels are showing', ifFalse: 'Labels are not showing'));
    ...
    if (value.runtimeType == DateTime && dateFormat != null) {
      properties
          .add(StringProperty('dateFormat', 'Formatted value is ${dateFormat!.format(value)}'));
    }
...
class _SfSliderState extends State<SfSlider> with TickerProviderStateMixin {
  ...
  SfSliderThemeData _getSliderThemeData(ThemeData themeData, bool isActive) {
    ...
    final double minTrackHeight =
        math.min(sliderThemeData.activeTrackHeight, sliderThemeData.inactiveTrackHeight);
    final double maxTrackHeight =
        math.max(sliderThemeData.activeTrackHeight, sliderThemeData.inactiveTrackHeight);
    sliderThemeData = sliderThemeData.copyWith(
      ...
      tooltipTextStyle: sliderThemeData.tooltipTextStyle ??
          themeData.textTheme.bodyLarge!.copyWith(color: themeData.colorScheme.surface),
      inactiveTrackColor: widget.inactiveColor ??
          sliderThemeData.inactiveTrackColor ??
          themeData.colorScheme.primary.withOpacity(0.24),
      activeTrackColor:
          widget.activeColor ?? sliderThemeData.activeTrackColor ?? themeData.colorScheme.primary,
      thumbColor: widget.activeColor ?? sliderThemeData.thumbColor ?? themeData.colorScheme.primary,
      activeTickColor:
          sliderThemeData.activeTickColor ?? themeData.colorScheme.onSurface.withOpacity(0.37),
      inactiveTickColor:
          sliderThemeData.inactiveTickColor ?? themeData.colorScheme.onSurface.withOpacity(0.37),
      ...
      activeMinorTickColor:
          sliderThemeData.activeMinorTickColor ?? themeData.colorScheme.onSurface.withOpacity(0.37),
      inactiveMinorTickColor: sliderThemeData.inactiveMinorTickColor ??
          themeData.colorScheme.onSurface.withOpacity(0.37),
      disabledActiveMinorTickColor: sliderThemeData.disabledActiveMinorTickColor ??
          themeData.colorScheme.onSurface.withOpacity(0.24),
      disabledInactiveMinorTickColor: sliderThemeData.disabledInactiveMinorTickColor ??
          themeData.colorScheme.onSurface.withOpacity(0.24),
      ...
      disabledInactiveDividerColor: sliderThemeData.disabledInactiveDividerColor ??
      ...
      disabledThumbColor: sliderThemeData.disabledThumbColor ??
          Color.alphaBlend(
              themeData.colorScheme.onSurface.withOpacity(0.38), themeData.colorScheme.surface),
...
class _RenderSlider extends RenderBaseSlider implements MouseTrackerAnnotation {
  _RenderSlider({
    ...
    _overlayAnimation =
        CurvedAnimation(parent: _state.overlayController, curve: Curves.fastOutSlowIn);

    _stateAnimation = CurvedAnimation(parent: _state.stateController, curve: Curves.easeInOut);

    _tooltipAnimation =
        CurvedAnimation(parent: _state.tooltipAnimationController, curve: Curves.fastOutSlowIn);
  ...
  void _drawTooltip(PaintingContext context, Offset thumbCenter, Offset offset,
    ...
    if (willDrawTooltip || shouldAlwaysShowTooltip) {
      ...
      final String tooltipText =
          tooltipTextFormatterCallback(actualText, getFormattedText(actualText));
      ...
      tooltipShape.paint(
          context, thumbCenter, Offset(actualTrackOffset.dx, tooltipStartY), textPainter,
          parentBox: this,
          sliderThemeData: sliderThemeData,
          paint: paint,
  ...
  void paint(PaintingContext context, Offset offset) {
    final Offset actualTrackOffset = sliderType == SliderType.horizontal
        ? Offset(offset.dx,
            offset.dy + (size.height - actualHeight) / 2 + trackOffset.dy - maxTrackHeight / 2)
        : Offset(offset.dx + (size.width - actualHeight) / 2 + trackOffset.dx - maxTrackHeight / 2,
            offset.dy);
    ...
    if (showLabels || showTicks || showDividers) {
      drawLabelsTicksAndDividers(
          context, trackRect, offset, thumbCenter, null, null, _stateAnimation, _value, null);
    }
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties
        .add(StringProperty('thumbSize', thumbShape.getPreferredSize(sliderThemeData).toString()));
    properties.add(StringProperty('activeDividerSize',
        dividerShape.getPreferredSize(sliderThemeData, isActive: true).toString()));
    properties.add(StringProperty('inactiveDividerSize',
        dividerShape.getPreferredSize(sliderThemeData, isActive: false).toString()));
    properties.add(
        StringProperty('overlaySize', overlayShape.getPreferredSize(sliderThemeData).toString()));
    properties
        .add(StringProperty('tickSize', tickShape.getPreferredSize(sliderThemeData).toString()));
    properties.add(StringProperty(
        'minorTickSize', minorTickShape.getPreferredSize(sliderThemeData).toString()));
  }
}
