...
class RenderBaseSlider extends RenderProxyBox with RelayoutWhenSystemFontsChangeMixin {
  ...
  RenderBaseSlider({
    ...
    thumbElevationTween = Tween<double>(begin: defaultElevation, end: tappedElevation);
  }
  ...
  TooltipTextFormatterCallback get tooltipTextFormatterCallback => _tooltipTextFormatterCallback;
  ...
  bool get isDateTime => _min.runtimeType == DateTime && _max.runtimeType == DateTime;

  // ignore: avoid_as
  double get actualMin => isDateTime ? _minInMilliseconds : _min.toDouble() as double;

  // ignore: avoid_as
  double get actualMax => isDateTime ? _maxInMilliseconds : _max.toDouble() as double;

  bool get isDiscrete => (_stepSize != null && _stepSize! > 0) || (_stepDuration != null);

  Size get _actualDividerSize => _dividerShape.getPreferredSize(_sliderThemeData);

  Size get actualTickSize => _tickShape.getPreferredSize(_sliderThemeData);

  Size get actualMinorTickSize => _minorTickShape.getPreferredSize(_sliderThemeData);

  double get maximumFontSize => math.max(
      _sliderThemeData.inactiveLabelStyle!.fontSize!, _sliderThemeData.activeLabelStyle!.fontSize!);

  // actualLabelSize is applicable only for horizontal sliders
  Size get actualLabelSize => Size.fromHeight(maximumFontSize);

  Rect get actualTrackRect => _trackShape.getPreferredRect(this, _sliderThemeData, Offset.zero);

  Size get actualThumbSize => _thumbShape.getPreferredSize(_sliderThemeData);

  Size get actualOverlaySize => _overlayShape.getPreferredSize(_sliderThemeData);

  double get actualTickHeight => _showTicks
      ? _sliderThemeData.tickSize!.height +
          (_sliderThemeData.tickOffset != null ? _sliderThemeData.tickOffset!.dy : 0)
      : 0;
  ...
  double get actualTickWidth => _showTicks
      ? _sliderThemeData.tickSize!.width +
          (_sliderThemeData.tickOffset != null ? _sliderThemeData.tickOffset!.dx : 0)
      : 0;

  double get actualMinorTickHeight => _minorTicksPerInterval > 0 ? actualMinorTickSize.height : 0;

  // actualMinorTickWidth is applicable only for vertical sliders
  double get actualMinorTickWidth => _minorTicksPerInterval > 0 ? actualMinorTickSize.width : 0;

  double get actualLabelHeight => _showLabels
      ? actualLabelSize.height * textPainter.textScaleFactor +
          (_sliderThemeData.labelOffset != null ? _sliderThemeData.labelOffset!.dy : 0)
      : 0;
  ...
  dynamic get semanticActionUnit =>
      isDateTime ? _stepDuration ?? adjustmentUnit : _stepSize ?? adjustmentUnit;
  ...
  double getMaxTrackHeight() {
    return math.max(_sliderThemeData.activeTrackHeight, _sliderThemeData.inactiveTrackHeight);
  }
  ...
  double getPositionFromValue(double value) {
    return sliderType == SliderType.horizontal
        ? getFactorFromValue(value) * actualTrackRect.width + actualTrackRect.left
        : actualTrackRect.bottom - getFactorFromValue(value) * actualTrackRect.height;
  }
  ...
  void _generateLabelsAndMajorTicksBasedOnInterval() {
    ...
    divisions = (isDateTime ? _getDateTimeDifference(_min, _max, _dateIntervalType) : _max - _min)
            .toDouble() /
        // ignore: avoid_as
        _interval as double;
    for (int i = 0; i <= divisions!; i++) {
      label = _labelFormatterCallback(currentValue, getFormattedText(currentValue));
      ...
      unformattedLabels
          ?.add(isDateTime ? valueInMilliseconds!.toDouble() : currentValue.toDouble());
      if (sliderType == SliderType.horizontal) {
        labelPosition = getFactorFromValue(isDateTime ? valueInMilliseconds : currentValue) *
  ...
  void _generateEdgeLabelsAndMajorTicks() {
    ...
    _visibleLabels.add(label);
    unformattedLabels?.add(isDateTime ? _min.millisecondsSinceEpoch.toDouble() : _min.toDouble());
    label = _labelFormatterCallback(_max, getFormattedText(_max));
    _visibleLabels.add(label);
    unformattedLabels?.add(isDateTime ? _max.millisecondsSinceEpoch.toDouble() : _max.toDouble());

    if (sliderType == SliderType.horizontal) {
      _majorTickPositions.add(getFactorFromValue(actualMin) * actualTrackRect.width);
      _majorTickPositions.add(getFactorFromValue(actualMax) * actualTrackRect.width);
    } else {
      _majorTickPositions.add(getFactorFromValue(actualMin) * actualTrackRect.height);
      _majorTickPositions.add(getFactorFromValue(actualMax) * actualTrackRect.height);
    }
  }
  ...
  void _generateDateTimeMinorTicks() {
    ...
    DateTime currentActualDate = _getNextDate(nextDate, _dateIntervalType, -_interval!);
    for (int i = 1; i <= majorTicksCount; i++) {
      ...
      final double intervalDiff =
          _getDateTimeDifference(currentActualDate, nextDate, _dateIntervalType) /
      ...
      if (intervalDiff == 1.0) {
        ...
        for (double j = 1; j <= _minorTicksPerInterval; j++) {
          final DateTime nextMinorDate = _getNextDate(currentActualDate, _dateIntervalType, j);
          minorTickPosition =
              _updateMinorTicksPosition(nextMinorDate.millisecondsSinceEpoch.toDouble());
          _minorTickPositions.add(minorTickPosition);
        }
      } else {
        final double minorPositionDiff =
            (nextDate.millisecondsSinceEpoch - currentActualDate.millisecondsSinceEpoch) /
                (_minorTicksPerInterval + 1);
  ...
  double _updateMinorTicksPosition(double value) {
    return getFactorFromValue(value) *
        (sliderType == SliderType.horizontal ? actualTrackRect.width : actualTrackRect.height);
  }
  ...
  int _getDateTimeDifference(DateTime min, DateTime max, DateIntervalType? intervalType) {
    assert(intervalType != null);
    final Duration diff = max.difference(min);
    switch (intervalType!) {
      case DateIntervalType.months:
        return ((max.year - min.year) * DateTime.monthsPerYear) + max.month - min.month;
  ...
  // ignore: missing_return
  DateTime _getNextDate(DateTime currentDate, DateIntervalType? intervalType, double interval) {
    assert(intervalType != null);
    switch (intervalType!) {
      case DateIntervalType.months:
      ...
      case DateIntervalType.hours:
        ...
        return DateTime(currentDate.year, currentDate.month, currentDate.day, currentDate.hour);
      case DateIntervalType.minutes:
  ...
  dynamic getValueFromPosition(double position) {
    ...
    if (sliderType == SliderType.horizontal) {
      ...
      valueFactor = (actualTrackRect.height - position) / actualTrackRect.height;
      if (!_isInversed) {
        valueFactor = 1.0 - valueFactor;
      }
    }
  ...
  dynamic getActualValue({dynamic value, double? valueInDouble}) {
    if (isDiscrete) {
      if (!isDateTime) {
        ...
        double factorValue =
            (getFactorFromValue(valueInDouble ?? value) * (maxMinDiff / _stepSize!)).round() /
        ...
        for (double i = actualMin; i < actualMax;) {
          ...
          final double currentDateInms = currentDate.millisecondsSinceEpoch.toDouble();
          ...
          if (clampedValue >= currentDateInms && clampedValue <= nextDateInms) {
            ...
            final bool shouldMoveNextInterval = clampedValue >= (currentDateInms + halfDateDiff);
  ...
  double getFactorFromCurrentPosition() {
    final double factor = (sliderType == SliderType.horizontal)
        ? ((mainAxisOffset - actualTrackRect.left) / actualTrackRect.width).clamp(0.0, 1.0)
        : ((actualTrackRect.bottom - mainAxisOffset) / actualTrackRect.height).clamp(0.0, 1.0);
    if (_isInversed) {
      return 1.0 - factor;
    } else {
      return factor;
  ...
  Rect getRectangularTooltipRect(TextPainter textPainter, Offset offset, Offset thumbCenter,
      Rect trackRect, SfSliderThemeData themeData) {
    final double rectangularTooltipHeight =
        textPainter.height + tooltipTextPadding.dy > minTooltipHeight
            ...
            : minTooltipHeight;
    final double halfTextWidth = textPainter.width + tooltipTextPadding.dx > minTooltipWidth
    ...
    final double top =
        thumbCenter.dy - rectangularTooltipHeight - offset.dy - tooltipTriangleHeight;
    final double bottom = thumbCenter.dy - offset.dy;

    return Rect.fromLTRB(left, top, right, bottom);
  }

  Rect getPaddleTooltipRect(TextPainter textPainter, double thumbRadius, Offset offset,
      Offset thumbCenter, Rect trackRect, SfSliderThemeData themeData) {
    final double paddleTooltipRadius = textPainter.height > minPaddleTopCircleRadius
        ? textPainter.height
        : minPaddleTopCircleRadius;
    final double topNeckRadius = paddleTooltipRadius - neckDifference;
    final double bottomNeckRadius =
        thumbRadius > minPaddleTopCircleRadius * moveNeckValue ? thumbRadius - neckDifference : 4.0;
    final double halfTextWidth = textPainter.width / 2 + textPadding;
    final double halfPaddleWidth =
        halfTextWidth > paddleTooltipRadius ? halfTextWidth : paddleTooltipRadius;
    final double shift = _getAdjustPaddleWidth(
        thumbCenter, offset, halfTextWidth - paddleTooltipRadius, paddleTooltipRadius, trackRect);
  ...
  double _getAdjustPaddleWidth(Offset thumbCenter, Offset offset, double halfTextWidth,
      double paddleTopCircleRadius, Rect trackRect) {
    ...
    final double rightEndPosition = trackRect.right + trackRect.left - offset.dx;
    // Moving the paddle top circle width from right to left.
    // When the tooltip leaves to the render box.
    shiftPaddleWidth = thumbCenter.dx + halfTextWidth + paddleTopCircleRadius > rightEndPosition
        ? thumbCenter.dx + halfTextWidth + paddleTopCircleRadius - rightEndPosition
        : shiftPaddleWidth;
  ...
  void drawLabelsTicksAndDividers(
    ...
    int dateTimePos = 0;
    bool isActive;
    final double dx = sliderType == SliderType.horizontal ? trackRect.left : trackRect.bottom;
    final double dy = sliderType == SliderType.horizontal ? trackRect.top : trackRect.left;
    final double halfTrackHeight =
        sliderType == SliderType.horizontal ? trackRect.height / 2 : trackRect.width / 2;
    if (startThumbCenter != null) {
      if (sliderType == SliderType.horizontal) {
        isActive = offset.dx >= startThumbCenter.dx && offset.dx <= endThumbCenter!.dx;
      } else {
        isActive = offset.dy <= startThumbCenter.dy && offset.dy >= endThumbCenter!.dy;
      }
    } else {
      if (sliderType == SliderType.horizontal) {
    ...
    final double dividerRadius =
        _dividerShape.getPreferredSize(_sliderThemeData, isActive: isActive).width / 2;
    ...
    // ignore: avoid_as
    final double maxRange = (isDateTime ? divisions : _max.toDouble()) as double;
    ...
    if (divisions != null && divisions! > 0) {
      while (textValue <= maxRange) {
        ...
        // Drawing ticks.
        if (_showTicks) {
          _drawTick(dx, tickPosition, dy, trackRect, dateTimePos, tickRadius, context, thumbCenter,
              startThumbCenter, endThumbCenter, value, values, stateAnimation);
        }
        ...
        if (_interval != null && _interval! > 0) {
          ...
          if (_minorTicksPerInterval > 0) {
            for (int j = 0; j < _minorTicksPerInterval; j++) {
              final double currentMinorTickPosition = _minorTickPositions[minorTickIndex];
              minorTickIndex++;
              _drawMinorTick(currentMinorTickPosition, trackRect, dx, dy, context, thumbCenter,
                  startThumbCenter, endThumbCenter, value, values, stateAnimation);
            }
          }
        ...
        if (_showLabels) {
          ...
          double offsetX =
              sliderType == SliderType.horizontal ? dx + tickPosition : dx - tickPosition;
          ...
          if (_labelPlacement == LabelPlacement.betweenTicks) {
            if (sliderType == SliderType.horizontal) {
              offsetX += ((dateTimePos + 1 <= divisions!
                      ...
                      tickPosition) /
                  2;
              if (_isInversed ? offsetX <= trackRect.left : offsetX - dx >= trackRect.width) {
                break;
              }
            } else {
              if (dateTimePos + 1 <= divisions!) {
                offsetX -= ((_majorTickPositions[dateTimePos + 1]) - tickPosition) / 2;
              } else {
                break;
              }
            }
          ...
          if (_edgeLabelPlacement == EdgeLabelPlacement.inside &&
            ...
            final Size labelSize = measureText(
              ...
              isInactive ? sliderThemeData.inactiveLabelStyle! : sliderThemeData.activeLabelStyle!,
            );
            if (sliderType == SliderType.horizontal) {
              if (dateTimePos == 0) {
          ...
          _drawLabel(dateTimePos, dx, tickPosition, trackRect, dy, context, thumbCenter,
              startThumbCenter, endThumbCenter, value, values, stateAnimation, offsetX);
        }
  ...
  void _drawTick(
    ...
    if (sliderType == SliderType.horizontal) {
      if (_majorTickPositions[dateTimePos] == 0.0) {
        actualTickOffset = Offset(dx + tickPosition + tickRadius, dy + trackRect.height) +
      ...
      if (_majorTickPositions[dateTimePos] == 0.0) {
        actualTickOffset = Offset(dy + trackRect.width, dx - tickPosition - tickRadius) +
            (_sliderThemeData.tickOffset ?? Offset.zero);
      }
      // Due to floating-point operations, last [_majorTickPosition] is greater
      // than [trackRect.height] or [trackRect.width]. This happens in some
    ...
    _tickShape.paint(context, actualTickOffset, thumbCenter, startThumbCenter, endThumbCenter,
        parentBox: this,
  ...
  void _drawMinorTick(
    ...
    if (currentMinorTickPosition <
            (sliderType == SliderType.horizontal ? trackRect.width : trackRect.height) &&
      ...
      _minorTickShape.paint(
          context, actualTickOffset, thumbCenter, startThumbCenter, endThumbCenter,
          parentBox: this,
          themeData: _sliderThemeData,
          currentValue: value,
          currentValues: values,
  ...
  void _drawDivider(
    ...
    if (sliderType == SliderType.horizontal) {
      if (_majorTickPositions[_dateTimePos] == 0.0) {
        dividerCenter = Offset(dx + tickPosition + dividerRadius, dy + halfTrackHeight);
      }
      // Due to floating-point operations, last [_majorTickPosition] is greater
      // than [trackRect.height] or [trackRect.width]. This happens in some
      // specific layouts alone. To avoid this, we limit it to 8 decimal points.
      ...
      if (_majorTickPositions[_dateTimePos] == 0.0) {
        dividerCenter = Offset(dy + halfTrackHeight, dx - tickPosition - dividerRadius);
      }
      // Due to floating-point operations, last [_majorTickPosition] is greater
      // than [trackRect.height] or [trackRect.width]. This happens in some
      // specific layouts alone. To avoid this, we limit it to 8 decimal points.
    ...
    _dividerShape.paint(context, dividerCenter, thumbCenter, startThumbCenter, endThumbCenter,
        parentBox: this,
        themeData: _sliderThemeData,
        currentValue: value,
        currentValues: values,
  ...
  void _drawLabel(
    ...
    _drawText(context, actualLabelOffset, thumbCenter, startThumbCenter, endThumbCenter, labelText,
        parentBox: this,
        themeData: _sliderThemeData,
        currentValue: value,
        currentValues: values,
  ...
  void _drawText(PaintingContext context, Offset center, Offset? thumbCenter,
    ...
    if (sliderType == SliderType.horizontal) {
      ...
      if (startThumbCenter != null) {
        if (!_isInversed) {
          isInactive = center.dx < startThumbCenter.dx || center.dx > endThumbCenter!.dx;
        } else {
          isInactive = center.dx > startThumbCenter.dx || center.dx < endThumbCenter!.dx;
        }
      } else {
      ...
      if (startThumbCenter != null) {
        if (!_isInversed) {
          isInactive = center.dy > startThumbCenter.dy || center.dy < endThumbCenter!.dy;
        } else {
          isInactive = center.dy < startThumbCenter.dy || center.dy > endThumbCenter!.dy;
        }
      } else {
    ...
    final TextSpan textSpan = TextSpan(
        ...
        // style: isInactive ? themeData.inactiveLabelStyle : themeData.activeLabelStyle,
        );
    textPainter.text = textSpan;
    textPainter.layout();
    if (sliderType == SliderType.horizontal) {
      textPainter.paint(context.canvas, Offset(center.dx - textPainter.width / 2, center.dy));
    } else {
      textPainter.paint(context.canvas, Offset(center.dx, center.dy - textPainter.height / 2));
    }
  }
    ...
    if (isDateTime) {
      if (_stepDuration == null) {
        return DateTime.fromMillisecondsSinceEpoch(
            (actualValue + semanticActionUnit).clamp(actualMin, actualMax).toInt());
      } else {
  ...
  void performLayout() {
    ...
    if (sliderType == SliderType.horizontal) {
      ...
      size = Size(
          constraints.hasBoundedWidth ? constraints.maxWidth : minTrackWidth + 2 * trackOffset.dx,
