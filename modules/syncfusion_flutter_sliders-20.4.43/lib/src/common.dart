...
typedef RangeSliderSemanticFormatterCallback = String Function(dynamic value, SfThumb thumb);

typedef RangeSelectorSemanticFormatterCallback = String Function(dynamic value, SfThumb thumb);
...
class SfRangeValues extends DiagnosticableTree {
  ...
  SfRangeValues copyWith({dynamic start, dynamic end}) {
    if (start != null && start.runtimeType == num && this.end.runtimeType == DateTime) {
      // ignore: avoid_as
      final double value = start as double;
      return SfRangeValues(DateTime.fromMillisecondsSinceEpoch(value.toInt()), end ?? this.end);
    } else if (end != null && end.runtimeType == num && this.start.runtimeType == DateTime) {
      // ignore: avoid_as
      final double value = end as double;
      return SfRangeValues(start ?? this.start, DateTime.fromMillisecondsSinceEpoch(value.toInt()));
    }
